...
awk -v section="$section" -v answer="$answer" -v pos="$answer_position" 'BEGIN { print "BEGIN_GENERAL"; for(i=1;i<=1022;i++) printf "NOTE=general-record-%04d\n",i; print "END_GENERAL"; print "BEGIN_" section; for(i=1;i<=2046;i++) if(i==pos) print "ANSWER=" answer; else if(i%211==0) printf "# comment about selected record %04d\n",i; else printf "NOTE=selected-record-%04d\n",i; print "END_" section; print "BEGIN_ARCHIVE"; for(i=1;i<=1022;i++) printf "NOTE=archive-record-%04d\n",i; print "END_ARCHIVE" }' > "$CASE_DIR/sections.txt"
