...
awk -v target="$target" -v pos="$target_position" -v encoded="$encoded" -v item="$item" -v group="$group" 'BEGIN { print "REFERENCE|DRAFT"; for(i=1;i<=4095;i++) if(i==pos) print target "|" encoded; else printf "REF-%06d|review-%s_waiting_for_%s_%d\n",i,item,group,10+i%80 }' > "$CASE_DIR/drafts.psv"
