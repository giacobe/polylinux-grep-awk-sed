...
awk -v target="$target" -v pos="$target_position" -v item="$item" -v answer="$answer" -v loc="$location" 'BEGIN { print "ID ITEM LOCATION STATUS"; for(i=1;i<=4095;i++) if(i==pos) printf "%s %s %s active\n",target,item,answer; else printf "ID-%05d %s %s-%02d %s\n",i,item,loc,10+i%80,(i%5==0?"pending":"ready") }' > "$CASE_DIR/inventory.txt"
