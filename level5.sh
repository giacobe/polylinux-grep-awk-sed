...
awk -v target="$target" -v pos="$target_position" -v owner="$owner" -v item="$item" -v place="$place" -v group="$group" -v loc="$location" 'BEGIN { print "REFERENCE|OWNER|ITEM|LOCATION|STATUS"; for(i=1;i<=4095;i++) if(i==pos) printf "%s|%s|%s|%s|active\n",target,owner,item,place; else printf "REF-%06d|%s-%02d|%s|%s-%02d|%s\n",i,group,10+i%80,item,loc,10+i%70,(i%6==0?"pending":"closed") }' > "$CASE_DIR/records.psv"
