...
awk -v r="$region" -v c="$category" -v o="$other" -v l="$location_name" -v p1="$p1" -v p2="$p2" -v p3="$p3" -v v1="$v1" -v v2="$v2" -v v3="$v3" 'BEGIN { print "# LOCATION|CATEGORY|STATUS|AMOUNT"; for (i=1; i<=8191; i++) if (i==p1) printf "%s|%s|complete|%d_units\n",r,c,v1; else if (i==p2) printf "%s|%s|complete|%d_units\n",r,c,v2; else if (i==p3) printf "%s|%s|complete|%d_units\n",r,c,v3; else if (i%4==0) printf "%s|%s|void|%d_units\n",r,c,100+i%800; else if (i%4==1) printf "%s|%s|complete|%d_units\n",r,o,100+i%800; else if (i%4==2) printf "%s-99|%s|complete|%d_units\n",l,c,100+i%800; else printf "%s|%s|invalid|%d_units\n",r,c,100+i%800 }' > "$CASE_DIR/report.psv"
