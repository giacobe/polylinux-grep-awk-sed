...
awk -v t="$target" \
    ...
    -v g="$group" \
'BEGIN{
    print "JOB,STATUS,OWNER,CATEGORY,VALUE";
    for(i=1;i<=4095;i++)
        if(i==vp)
...
write_readme "Read the target job in data/TASK.txt. data/jobs.csv contains 4,096 lines. Build a pipeline that uses grep to keep the target's VALID record
