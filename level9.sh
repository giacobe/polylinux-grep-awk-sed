...
awk -v target="$target" -v vp="$valid_position" -v xp="$void_position" -v owner="$owner" -v cat="$category" -v value="$value" -v group="$group" 'BEGIN { print "JOB,STATUS,OWNER,CATEGORY,VALUE"; for(i=1;i<=4095;i++) if(i==vp) printf "%s,VALID,%s,%s,%d\n",target,owner,cat,value; else if(i==xp) printf "%s,VOID,%s,%s,%d\n",target,owner,cat,value+20; else printf "JOB-%05d,%s,%s-%02d,%s,%d\n",i,(i%9==0?"INVALID":"VALID"),group,10+i%80,cat,100+i%700 }' > "$CASE_DIR/jobs.csv"
